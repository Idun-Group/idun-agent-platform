...
class ToolDefinition(BaseModel):
    ...
    schema_: Optional[Dict[str, Any]] = Field(
        ...
        alias="schema",
        description="The schema of the tool's input/output, often a JSON schema.",
    )
...
class Agent(BaseModel):
    ...
    config: Dict[str, Any] = Field(
        default_factory=dict,
        description="Framework-specific configuration for the agent.",
    )
    llm_config: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="LLM configuration for the agent."
    )
    tools: List[ToolDefinition] = Field(
        default_factory=list, description="A list of tools available to the agent."
    )
    # agent_path is now optional, as it can be derived from the uploaded file.
    agent_path: Optional[str] = None
    agent_variable: Optional[str] = None
