...
async def upload_agent(
    ...
    try:
        ...
        dockerfile_content = dockerfile_template.replace(
            "AGENT_ENTRYPOINT", uvicorn_entrypoint
        )
        ...
        image, build_logs = docker_client.images.build(
            path=agent_dir, tag=f"{service_name}:latest", rm=True
        )
        ...
        print(
            f"Container {container.id} started successfully for service {service_name}."
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Docker build failed:\n{error_logs}"
        )
    except docker.errors.APIError as e:
        shutil.rmtree(agent_dir, ignore_errors=True)
        raise HTTPException(
            status_code=500, detail=f"Docker API failed to start container: {e}"
        )
    except json.JSONDecodeError:
        shutil.rmtree(agent_dir, ignore_errors=True)
        raise HTTPException(
            status_code=400, detail="Invalid JSON format for 'config' field."
        )
...
@router.post(
    "/", response_model=Agent, status_code=status.HTTP_201_CREATED, deprecated=True
)
    ...
    try:
        ...
        if file:
            if not agent_variable or not agent_path:
                raise HTTPException(
                    status_code=400,
                    detail="With a file upload, 'agent_variable' and 'agent_path' (main file in zip) are required.",
                )
            ...
            if framework_type in ["langgraph"]:
                raise HTTPException(
                    status_code=400,
                    detail="'agent_path' and 'agent_variable' are required for this framework type when not uploading a file.",
                )
...
async def delete_agent(agent_id: str):
    ...
    if not sqlite_db.delete_agent_from_db(agent_id):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Agent not found in database."
        )
    return
...
async def run_agent_no_stream(agent_id: str, request: ChatRequest):
    ...
    try:
        ...
        return ChatResponse(
            agent_id=agent_id, session_id=request.session_id, response=response_content
        )
