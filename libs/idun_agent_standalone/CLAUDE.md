...
**Attribute projection.** The exporter's `_span_to_row` composes the engine helper `idun_agent_engine.observability.projection.project_to_openinference` between attribute truncation and the kind read. ADK-shape spans (kind=INTERNAL, `gen_ai.*` + `gcp.vertex.agent.*` keys) get their OpenInference projection (`openinference.span.kind`, `llm.model_name`, `llm.token_count.*`, `tool.name`, `input.value`, `output.value`) so the row-build path materialises model / tokens / cost columns identically for ADK and LangGraph. LangGraph spans (already OpenInference-shaped) pass through bit-identical via the helper's early-return. Pre-projection DB rows retain their source-only shape; the UI's `inferKind` fallback at `services/idun_agent_standalone_ui/components/traces/_kind.ts` covers them.
