...
class ConfigBuilder:
    ...
    async def initialize_agent_from_config(engine_config: EngineConfig) -> BaseAgent:
        ...
        Returns:
            BaeAgent: Initialized agent instance
        ...
        log_operation(
            logger,
            "INFO",
            "agent_initialization_start",
            f"Initializing {agent_type} agent",
            agent_type=agent_type,
            agent_config=agent_config_obj.model_dump()
            if hasattr(agent_config_obj, "model_dump")
            else agent_config_obj,
        )
    ...
    def load_from_file(config_path: str = "config.yaml") -> EngineConfig:
        ...
        try:
            ...
            log_operation(
                ...
                raise YAMLError(
                    f"Failed to convert guardrails configuration: {e}"
                ) from e
