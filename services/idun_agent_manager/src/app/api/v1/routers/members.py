...
async def _auto_add_default_project(
    workspace_id: UUID,
    user_id: UUID,
    role: str,
    assigned_project_ids: set[UUID],
    session: AsyncSession,
) -> None:
    """Auto-add user to the workspace's default project if not already assigned.

    Skipped if *assigned_project_ids* already contains the default project.
    """
    ...
    if default_project.id in assigned_project_ids:
        return  # Explicit assignment takes precedence
...
async def add_member(
    ...
    The user is automatically added to the default project as Reader unless
    an explicit assignment for the default project is included in the request.
    ...
    if existing_user is not None:
        ...
        # Create project memberships for any explicit assignments
        ...
        for assignment in request.project_assignments:
            ...
            # Owners always get admin, regardless of what was requested
            effective_role = (
        ...
        # Auto-add to projects based on role
        if request.is_owner:
            # Owners get admin on ALL projects
            all_projects_stmt = select(ProjectModel).where(
                ProjectModel.workspace_id == workspace_id,
            )
            all_projects = (
                (await session.execute(all_projects_stmt)).scalars().all()
            )
            for proj in all_projects:
                if proj.id not in assigned_project_ids:
                    pm = ProjectMembershipModel(
                        id=uuid4(),
                        project_id=proj.id,
                        user_id=existing_user.id,
            ...
            # Non-owners get Reader on default project (unless already assigned)
            await _auto_add_default_project(
                workspace_id,
                existing_user.id,
...
async def accept_invitation(
    ...
    workspace membership, materialises project assignments from the
    invitation, and auto-adds to the default project as Reader (unless
    an explicit assignment overrides it). Deletes the invitation on success.
    ...
    # Create project memberships from invitation assignments
    assigned_project_ids: set[UUID] = set()
    for ip in ip_rows:
        # Skip if the project was deleted since the invitation was created
        ...
        # Owners always get admin, regardless of invitation role
        effective_role = (
            ProjectRole.ADMIN.value
        ...
        session.add(pm)
        assigned_project_ids.add(ip.project_id)
