"""Auth router – Google OIDC SSO + session cookie management.

Endpoints:
    GET  /login     – Redirect to Google authorization endpoint
    GET  /callback  – Exchange code for tokens, set session cookie, redirect to frontend
...
async def callback(
    ...
    if not userinfo:
        ...
        try:
            resp = await oauth.google.get("https://openidconnect.googleapis.com/v1/userinfo", token=token)  # type: ignore[union-attr]
            userinfo = resp.json()
        except Exception:
            pass
