...
class TestGetPromptsConfig:
    async def test_config_returns_assigned_prompts(
        self,
        authed_client: AsyncClient,
        seeded_prompt: ManagedPromptModel,
        seeded_agent: ManagedAgentModel,
        ...
        resp = await authed_client.get(
            ...
            headers={"auth": f"Bearer {seeded_agent.agent_hash}"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert len(data) == 1
        assert data[0]["prompt_id"] == "system-prompt"

    async def test_config_invalid_key(self, authed_client: AsyncClient):
        resp = await authed_client.get(
            "/api/v1/prompts/config",
            headers={"auth": "Bearer invalid-key"},
        )
        assert resp.status_code == 404

    async def test_config_missing_bearer(self, authed_client: AsyncClient):
        resp = await authed_client.get(
            "/api/v1/prompts/config",
            headers={"auth": "not-bearer"},
        )
        assert resp.status_code == 401

    async def test_config_no_assignments_returns_empty(
        self,
        authed_client: AsyncClient,
        seeded_agent: ManagedAgentModel,
    ):
