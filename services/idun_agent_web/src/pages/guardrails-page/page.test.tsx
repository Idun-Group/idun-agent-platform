...
describe('GuardrailsPage — Reader with no guardrails (empty state)', () => {
    beforeEach(() => {
        vi.clearAllMocks();
    ...
    it('renders reader-aware empty-state copy (passive title, ask-admin description)', async () => {
        renderPage();
        await vi.waitFor(() => {
            expect(screen.queryByText(/No guardrails configured in Test Project/i)).not.toBeNull();
            expect(screen.queryByText(/Ask a contributor or admin to add one/i)).not.toBeNull();
        });
        // Writer-flavored title must not appear for readers.
        expect(screen.queryByText('Select a guardrail to get started')).toBeNull();
    });
});

describe('GuardrailsPage — Admin with no guardrails (empty state)', () => {
    beforeEach(() => {
        vi.clearAllMocks();
        mockUseWorkspace.mockReturnValue(workspaceHook() as ReturnType<typeof useWorkspace>);
    ...
    it('renders the write-flavored empty-state title', async () => {
        renderPage();
        await vi.waitFor(() => {
            expect(screen.queryByText('Select a guardrail to get started')).not.toBeNull();
        });
    });
});
