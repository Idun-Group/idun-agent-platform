...
describe('MCPPage — Reader with no servers (empty state)', () => {
    beforeEach(() => {
        vi.clearAllMocks();
    ...
    it('renders reader-aware empty-state copy (passive title, ask-admin description)', async () => {
        renderPage();
        await vi.waitFor(() => {
            expect(screen.queryByText(/No MCP servers in Test Project yet/i)).not.toBeNull();
        ...
        expect(screen.queryByText('Add an MCP server to get started')).toBeNull();
    });
});

describe('MCPPage — Admin with no servers (empty state)', () => {
    beforeEach(() => {
        vi.clearAllMocks();
        mockUseWorkspace.mockReturnValue(workspaceHook() as ReturnType<typeof useWorkspace>);
    ...
    it('renders the write-flavored empty-state title', async () => {
        renderPage();
        await vi.waitFor(() => {
            expect(screen.queryByText('Add an MCP server to get started')).not.toBeNull();
        });
    });
});
