...
import { fetchApplications, createApplication } from '../../services/applications';
...
const MemoryPage = () => {
  ...
  const handleCreateMemory = async (data: { framework: string, provider: string, config: any, name: string, description: string }) => {
      setCreateError(null);
      try {
          await createApplication({
              name: data.name,
              type: data.provider as any, // This needs to map to valid AppType
              category: 'Memory',
              config: data.config,
              imageUrl: '/img/database.svg' // Placeholder
          });
          toast.success("Memory store created successfully");
          setIsCreateModalOpen(false);
          loadMemories();
      } catch (error: any) {
          console.error("Failed to create memory", error);
          // Check if error response has detailed message
          let errorMessage = error.message || "Failed to create memory store";
          ...
          // Pass error back to modal to display inline, and stop modal from closing/loading state
