...
<Tabs>
  ...
  <Tab title="Manager UI">
    Open an agent in the Manager UI and go to the **Overview** tab to manage its lifecycle.

    ![Agent detail overview tab](/images/ui/agents-detail-overview.png)

    From this tab you can:

    - **Toggle the status** to enable or disable the agent without deleting it
    - **Run a health check** to verify the agent engine is reachable and responding
    - **Restart the agent** to apply config changes or recover from an error state

    Status changes are reflected immediately in the agents list and propagated to any connected engine instances on their next config fetch.
  </Tab>
</Tabs>
...
<Cards>
  <Card title="Google Cloud Platform" icon="google" href="/deployment/gcp">
    Cloud Run for Manager and UI, Cloud SQL for PostgreSQL. Contact us for Terraform modules.
  </Card>
</Cards>
