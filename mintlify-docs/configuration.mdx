...
**`agent`** -- Framework type and framework-specific settings. The `config` fields change based on the `type` value. Supported types: `LANGGRAPH`, `ADK`, `HAYSTACK`. See the [frameworks overview](/frameworks/overview) for per-framework config details.
