...
ADK agents use **session services** and **memory services** to manage conversational context. These two services work together: session services handle conversation state for individual interactions, while memory services provide long-term knowledge storage that spans multiple sessions.
