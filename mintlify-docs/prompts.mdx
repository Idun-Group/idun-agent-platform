...
<Tabs>
  ...
  <Tab title="Manager UI">
    Navigate to **Prompts** in the sidebar to manage all prompt versions across your workspace.

    ![Prompts list](/images/ui/prompts-list.png)

    Click **New Prompt** to open the prompt editor.

    ![Create prompt form](/images/ui/prompts-create-form.png)

    The editor uses Monaco with Jinja2 variable detection. Variables found in the content are displayed as pills below the editor so you can verify all placeholders before saving.

    After creating a prompt, assign it to one or more agents directly from the prompt version row using the link icon. Assigned prompts are injected into the agent's engine config automatically when the engine fetches its configuration.
  </Tab>
</Tabs>
