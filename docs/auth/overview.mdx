...
description: "Configure user authentication with username/password or OIDC SSO (Google, Microsoft), and manage access with role-based access control."
keywords: ["authentication", "authorization", "SSO", "OIDC", "RBAC", "roles", "Google OAuth", "Google OIDC", "Microsoft", "Azure AD", "Entra ID"]
---
...
<Tabs>
  ...
  <Tab title="OIDC SSO">
    Enable SSO by setting `AUTH__DISABLE_USERNAME_PASSWORD=true` and configuring one or more OIDC providers. Each provider is registered automatically when its `client_id` env var is set.

    - `GET /api/v1/auth/providers` lists enabled providers
    - `GET /api/v1/auth/login/{provider}` redirects to the provider's authorization page
    ...
    You can configure both providers simultaneously. The login page will show buttons for each enabled provider.
  </Tab>
</Tabs>
...
For per-agent SSO, the engine validates JWTs against any OIDC-compliant provider's JWKS endpoint.
