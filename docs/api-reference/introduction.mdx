---
title: "Introduction"
description: "HTTP API of every idun-agent-standalone deployment. 42 endpoints, auto-generated from the live FastAPI app."
keywords: ["api", "rest", "openapi", "standalone", "endpoints", "reference"]
boost: 2
---

Every `idun-agent-standalone` deployment exposes the same HTTP API. This reference is generated from the live FastAPI app on every release, so every shape on every page matches what the server actually returns.
...
This reference reflects a snapshot of `GET /openapi.json` from a running standalone. On every release we re-export the spec to `docs/standalone/openapi.json`; the API pages regenerate automatically.
