...
Admin-panel auth (`IDUN_ADMIN_AUTH_MODE=password`) gates the `/admin/*` UI and admin REST. It does **not** gate the `/agent/*` runtime routes. To require an OIDC JWT on the agent API, enable per-agent SSO. See [SSO](/auth/sso). {/* VERIFY: gating scope (admin-only) sourced from libs/idun_agent_standalone/src/idun_agent_standalone/services/auth.py and the SSO dependency in libs/idun_agent_engine/src/idun_agent_engine/server/dependencies.py; recheck before each release that no admin route is left ungated and no agent route is gated by the session cookie. */}
