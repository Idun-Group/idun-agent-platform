...
Admin-panel auth (`IDUN_ADMIN_AUTH_MODE=password`) gates the `/admin/*` UI and admin REST. It does **not** gate the `/agent/*` runtime routes. To require an OIDC JWT on the agent API, enable per-agent SSO. See [Agent route protection](/auth/overview).
...
- [Agent route protection](/auth/overview): require an OIDC JWT on the agent API
