...
**`server`** -- HTTP server binding. Exposes `api.port` (default: `8000`). CORS allows all origins. The engine adds `Access-Control-Allow-Private-Network: true` so hosted UIs can reach local agents. Two optional fields control [exposing the agent as an MCP server](/tool-governance/overview#expose-an-agent-as-an-mcp-server): `as_mcp` (boolean, default `true`) serves a `/mcp` endpoint that exposes the agent as a single MCP tool, and `mcp_description` (string) customizes the tool description.
...
<Tabs>
  <Tab title="Manager UI">
    Open an agent and go to the **Configuration** tab. This shows the full materialized engine config that the agent receives from the platform when it connects.

    Toggle between JSON and YAML views. Use the Copy button to grab the config for local testing or debugging. Note that `${ENV_VAR}` references aren't resolved here; the engine resolves them at load time.

    ![Agent detail Configuration tab](/images/ui/agents-detail-config.png)
  </Tab>
