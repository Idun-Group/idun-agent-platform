...
- `/health` now reflects engine assembly state — returns `agent_ready: bool` and `status: "degraded"` when configured agents failed to come online.
...
1. Uninstall any separate `idun-agent-standalone` wheel — it is now bundled in the engine wheel.
