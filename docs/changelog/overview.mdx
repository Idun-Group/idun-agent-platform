...
<Update label="v0.6.4" description="2026-09-22">
  Patch release. Telemetry now reports to PostHog EU.

  ### Changed

  - **Telemetry now reports to PostHog EU.** Product telemetry from the engine, the standalone backend, and the bundled UI is sent to PostHog Cloud EU (`eu.i.posthog.com`) instead of the US project, so the data lands in the EU region. Telemetry stays on by default and the off-switch is unchanged: `IDUN_TELEMETRY_ENABLED=false`. The event set and the masking rules are untouched — only the destination moved. Upgrade to this release to report to the EU project; older installs keep reporting to the US one.
</Update>
