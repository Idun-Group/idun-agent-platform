...
Métriques d'utilisation minimales et anonymes + rediffusion de session masquée via PostHog, hébergé dans l'UE (`eu.i.posthog.com`). Aucun contenu de message, aucune PII au-delà de l'e-mail après connexion (définir `IDUN_TELEMETRY_IDENTIFY_USERS=false` pour désactiver). [Voir le code source](https://github.com/Idun-Group/idun-agent-platform/blob/develop/libs/idun_agent_engine/src/idun_agent_engine/telemetry/telemetry.py). Désactiver : `IDUN_TELEMETRY_ENABLED=false`. Désactiver uniquement la rediffusion : `IDUN_TELEMETRY_SESSION_REPLAY=false`.
