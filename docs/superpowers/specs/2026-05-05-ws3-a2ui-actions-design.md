...
3. **No agent-side state-schema coupling.** Agents do **not** declare an `idun` field in their `TypedDict` state. LangGraph forwards extra keys from the initial input to nodes; nodes that don't read them ignore them. The smoke-test agent's `class State(TypedDict): messages: ...` works as-is.
4. **No checkpointer staleness.** Because `idun` isn't in the declared state schema, the checkpointer doesn't persist it — every turn's `idun` value comes only from that turn's `forwardedProps`. Self-cleaning by construction.
