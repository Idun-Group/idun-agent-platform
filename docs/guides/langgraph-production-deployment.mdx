...
Idun supports five observability providers, attached one at a time per agent:
...
The standalone admin UI attaches a single external provider per agent. Independently, the local trace store at `/admin/traces/` always captures every run alongside whichever external provider is configured; see [Local trace store](/observability/traces).
