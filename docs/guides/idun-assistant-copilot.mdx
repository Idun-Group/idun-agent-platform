---
title: "How Idun makes building copilots a breeze"
sidebarTitle: "Building a copilot (idun-assistant)"
description: "Walk through idun-assistant, Idun's in-house dev copilot. About 180 lines of agent code plus a few clicks in the admin panel, and you get a tool-using LangGraph agent ready for production."
keywords: ["copilot", "LangGraph", "MCP", "idun-assistant", "case study", "production agent", "Atlassian", "GitHub", "Google Workspace", "Langfuse"]
---

idun-assistant is Idun's in-house dev copilot. A LangGraph agent that reads our docs, queries Jira and Confluence, opens GitHub issues, drafts Google Docs, and sends mail. End-to-end, the agent code is ~180 lines of Python; everything else (tools, prompts, memory, observability, auth) is wired through the standalone admin panel.

This guide walks the path from "I have an agent" to "I have a protected, observable copilot," then runs a real multi-tool task in chat.

## The demo we'll run

After setup, we'll send this to the bot:
...
`agent/agent.py` is a normal LangGraph workflow. A single ReAct loop. Two nodes, one conditional edge, no Idun-specific scaffolding.
...
_model = _make_model()
SYSTEM_PROMPT = (get_prompt("system_prompt") or PromptConfig.empty()).content
...
|---|---|---|
| `idun-docs` | Streamable HTTP at `https://docs.idunplatform.com/mcp` | Search Idun's own docs. |
...
The default scaffold uses in-memory checkpointing, which loses every conversation when the process restarts. For idun-assistant we want conversations to survive a reboot. Open `/admin/memory/` and pick SQLite.
...
idun-assistant is an internal tool. We want everyone with an `@idun-group.com` Google account to use it, and no one else. Open `/admin/sso/` and configure Google OIDC with a domain allowlist.
