...
The Idun standalone UI emits the events below through PostHog. Events are sent to PostHog Cloud EU (`eu.i.posthog.com`). The single off-switch is `IDUN_TELEMETRY_ENABLED=false`, which also kills Python engine telemetry.
