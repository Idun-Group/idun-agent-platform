...
Both reference handlers define a static `_resolve_env` helper for the `${ENV_VAR}` syntax users can put in their YAML:
...
Copy this verbatim. It lets users keep secrets in env vars and reference them in `config.yaml`:
