...
<AccordionGroup>
  <Accordion title="`agent_not_ready`: HTTP 503 from `/agent/*`">
  ...
  <Accordion title="`boot engine layer skipped, admin only mode` log line">
  ...
  <Accordion title="`RELOAD_FAILED`: the engine refused to reload">
    **Cause.** One of:
  ...
  <Accordion title="`Could not load spec for module at <X>` from the ADK adapter">
    **Symptom.** Engine startup fails with:

    ```
    ImportError: Could not load spec for module at <X>
    ValueError: Failed to load agent from <X>:<variable>: ...
    ```
    ...
    **Fix.** Rewrite the path in file-path form:
    ...
    Relative paths resolve against the working directory `idun serve` was launched from. Absolute paths also work.
  </Accordion>

  <Accordion title="`config.yaml` edits don't show up after restart">
    **Symptom.** You edit `config.yaml` (change a model, add an MCP server, tweak a guardrail), restart with `idun serve` or `idun setup`, and nothing changes in the running agent.
    ...
    **Fix.** Edit through the admin panel at `/admin/` instead. Saves there flow through the three-round validation + reload pipeline and apply immediately (subject to the `RESTART_REQUIRED` rules under "Reload pipeline outcomes" above).
    ...
    If you really want to re-seed from YAML, you need to clear the relevant DB rows first. For a clean restart, stop the server, delete `idun_standalone.db` (or, on Postgres, truncate the seeded tables), then run `idun setup` followed by `idun serve`.
  </Accordion>
