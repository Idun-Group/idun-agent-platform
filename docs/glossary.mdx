...
The first boot seeds the local DB from `IDUN_CONFIG_PATH` (the YAML inside the scaffold). After that, the DB is the source of truth and admin edits persist across restarts. Visit `http://127.0.0.1:8000/` for the chat UI, `/admin/` to edit config, `/admin/traces/` to inspect AG-UI events.
