...
`make build-standalone-all` runs `pnpm build` for the Next.js UI and `uv build` for the wheel; the UI static export ends up bundled inside the wheel. See the [CLI reference](/cli/overview) for every flag and env var.
