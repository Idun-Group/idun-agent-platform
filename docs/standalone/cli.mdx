...
| Flag | Default | Description |
|---|---|---|
| `--source file` | required | Load config from a local YAML file. |
...
| `IDUN_ADMIN_PASSWORD_HASH` | empty | Bcrypt admin hash. Consulted only on first boot to seed the admin row. Required when `auth_mode = password`. Generate with `idun hash-password`. |
